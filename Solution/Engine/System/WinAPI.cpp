...
WinAPI::WinAPI()
	: windowSize(POINT{ .x = window_width, .y = window_height }),
	systemWindowSize(POINT{ .x = window_width, .y = window_height })
{
	...
	systemWindowSize = POINT{ .x = displayWid, .y = displayHei };
...
bool WinAPI::setWindowSize(int sizeX, int sizeY, const POINT* pos, bool bRepaint)
{
	...
	windowSize = POINT{ .x = sizeX, .y = sizeY };
