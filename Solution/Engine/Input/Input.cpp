...
{
	float tmp_x = float(x);
	tmp_x /= float(WinAPI::getInstance()->getWindowSize().x);
	tmp_x *= float(WinAPI::getInstance()->getSystemWindowSize().x);

	float tmp_y = float(y);
	tmp_y /= float(WinAPI::getInstance()->getWindowSize().y);
	tmp_y *= float(WinAPI::getInstance()->getSystemWindowSize().y);

	POINT tmpPos = { LONG(tmp_x), LONG(tmp_y) };
