...
public:
	...
	/// @brief 通常はcalcMousePosFromSystemを使用してください（フルスクリーンではうまくいかないので）
	...
	inline float calcMousePosXFromSystem() const
	{
		float n = float(mousePos.x);
		n /= float(WinAPI::getInstance()->getSystemWindowSize().x);
		n *= float(WinAPI::getInstance()->getWindowSize().x);
		return n;
	}
	inline float calcMousePosYFromSystem() const
	{
		float n = float(mousePos.y);
		n /= float(WinAPI::getInstance()->getSystemWindowSize().y);
		n *= float(WinAPI::getInstance()->getWindowSize().y);
		return n;
	}

	inline DirectX::XMFLOAT2 calcMousePosFromSystem() const
	{
		return DirectX::XMFLOAT2(float(calcMousePosXFromSystem()),
								 float(calcMousePosYFromSystem()));
	}
