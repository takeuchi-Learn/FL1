...
public:
	...
	/// @brief レイと直方体(AABB)の当たり判定
	/// @param ray レイ
	/// @param aabb 直方体(AABB
	/// @param inter 交点(出力用)
	/// @return 交差しているか否か
	static bool CheckRay2AABB(const Ray& ray, const AABB& aabb, DirectX::XMVECTOR* inter = nullptr);

	inline static bool CheckHit(const Ray& ray, const AABB& aabb) { return CheckRay2AABB(ray, aabb); }
	inline static bool CheckHit(const AABB& aabb, const Ray& ray) { return CheckRay2AABB(ray, aabb); }
	...
	/// @param inter 交点(出力用)
	/// @return 交差しているか否か
	static bool CheckRay2Capsule(const Ray& ray, const Capsule& capsule, DirectX::XMVECTOR* inter = nullptr);

	inline static bool CheckHit(const Ray& ray, const Capsule& capsule) { return CheckRay2Capsule(ray, capsule); }
	inline static bool CheckHit(const Capsule& capsule, const Ray& ray) { return CheckRay2Capsule(ray, capsule); }

	/// @brief 球と直方体(AABB)の当たり判定
	/// @param sphere 球
	/// @param aabb 直方体(AABB)
	/// @return 衝突しているか否か
